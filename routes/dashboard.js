...
router.get('/inventory', ensureAuthenticated, ensureAdmin, function(req, res, next)
{
  Product.getAllProducts(function(e, products)
  {
    ...
    {
      checkVariants(products, function(data)
      {
...
function checkVariants(products, callback)
{
  for (let i = 0; i < products.length; i++)
  {
    Variant.getVariantProductByID(products[i]._id, function(e, variants){
      if (variants.length > 0)
      {
        products[i]["variant"] = variants.length
      }
    })
  }
  callback(products)
