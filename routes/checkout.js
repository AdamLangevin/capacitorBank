...
router.post('/checkout-process', function(req, res) {
  ...
  let createPayment = {
    ...
    "redirect_urls": {
      "return_url":"http://134.117.216.150:3000/checkout/checkout-success",
      "cancel_url":"http://134.117.216.150:3000/checkout/checkout-cancel"
    },
    "transactions": [{
        "amount": {
            "currency": "CAD",
        ...
        "description": "Capacitor Bank payment"
    }]
  };
  ...
  paypal.payment.create(createPayment, function(err, payment){
    ...
    if(err){
      ...
      for(let i=0; i<payment.links.length;   i++){
        let ln = payment.links[i];
        if(ln.method === 'REDIRECT'){
          link = ln.href;
...
router.get('/checkout-success', ensureAuthenticated, function(req, res) {
  ...
  paypal.payment.execute(payID, payerID, function(err, payment){
    if(err){
      ...
      if(payment.state == 'approved'){
        ...
        res.locals.session.cart = cart.clearCart();
        res.render('checkoutSuccess', {
          title: 'Successful',
          containerWrapper: 'container'
        });
        let address = payment.payer.payer_info.shipping_address.line1 + ", " +
